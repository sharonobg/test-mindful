...
const getBothWTotals = async (props) => {
    try{
        const res = await fetch("http://localhost:3000/api/spendingplan-both",{
           cache: 'no-store',
           method: "GET",
           headers: headers(),
        });
...
//const mycategoriesdata = getPlans();
   ...
   //transactiontotals)
   console.log('SPCategoryView getboth',getboth)
   
    return(
       <>
          <pre>GET Plans:{JSON.stringify(spendingplannew, null, 2)}</pre>
          <pre>GET BOTH:{JSON.stringify(getboth, null, 2)}</pre>
       ...
       <div className="flex flex-row  w-full min-h-[50%] bg-white">
        ...
        {spendingplannew?.length > -1 ? (spendingplannew.map((spendingplan,index) =>
            
        <div key={index} className="spkey">
        { spendingplan.month === 2 && spendingplan.year == 2024 &&  ( 
                    <>
        {spendingplan.transactlookup.mycategories.map((lookup,index) =>
